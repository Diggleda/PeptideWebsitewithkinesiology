#!/usr/bin/env node
/*
Backfill MySQL `trufusion_orders.total` to the true grand total and normalize payload totals.

Why:
  Some historical rows stored the items subtotal in `trufusion_orders.total`. Downstream reporting
  expects `total` to be the full order total (subtotal - discounts + shipping + tax).

Usage (dry-run):
  node scripts/backfill_trufusion_orders_total.js --limit 200

Apply:
  node scripts/backfill_trufusion_orders_total.js --apply --limit 200
*/
...
const computeGrandTotal = (order) => {
  ...
  const itemsSubtotal = toNumber(
    order.itemsSubtotal ?? order.items_subtotal ?? order.itemsTotal ?? order.items_total ?? order.trufusion_items_subtotal,
    NaN,
  );
  const shippingTotal = toNumber(order.shippingTotal ?? order.shipping_total ?? order.trufusion_shipping_total, 0);
...
const main = async () => {
  ...
  logger.info({ mode, scanned, updated }, 'trufusion_orders total backfill complete');
  // eslint-disable-next-line no-console
  console.log(`${mode}: scanned=${scanned} updated=${updated}`);
};

main().catch((error) => {
  logger.error({ err: error }, 'trufusion_orders total backfill failed');
  // eslint-disable-next-line no-console
