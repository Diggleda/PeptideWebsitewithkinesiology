...
@blueprint.get("/admin/ups-sync-status")
@require_auth
def admin_ups_sync_status():
    def action():
        role = (g.current_user.get("role") or "").lower()
        if role != "admin":
            err = ValueError("Admin access required")
            setattr(err, "status", 403)
            raise err
        from ..services.ups_status_sync_service import get_status
...
@blueprint.post("/admin/sync-ups-statuses")
@require_auth
def admin_run_ups_sync_now():
    payload = request.get_json(force=True, silent=True) or {}
    ...
    def action():
        ...
        if role != "admin":
            ...
            raise err
        ignore_cooldown = bool(payload.get("ignoreCooldown") is True or payload.get("ignore_cooldown") is True)
        from ..services.ups_status_sync_service import run_sync_once, get_status
