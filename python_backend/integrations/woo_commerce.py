...
def _build_woo_address(
    ...
    *,
    ...
    explicit_recipient_name = _first_non_empty_text(
        address_dict.get("recipientName"),
        address_dict.get("recipient_name"),
        address_dict.get("orderRecipientName"),
        address_dict.get("order_recipient_name"),
        address_dict.get("pickupRecipientName"),
        address_dict.get("pickup_recipient_name"),
    )
    source_full_name = _first_non_empty_text(
        explicit_recipient_name,
        address_dict.get("fullName"),
        address_dict.get("name"),
    )
    full_name = _first_non_empty_text(
        source_full_name,
        fallback_dict.get("recipientName"),
        fallback_dict.get("recipient_name"),
        fallback_dict.get("orderRecipientName"),
        fallback_dict.get("order_recipient_name"),
        fallback_dict.get("pickupRecipientName"),
        fallback_dict.get("pickup_recipient_name"),
    ...
    if explicit_recipient_name:
        first_name, last_name = _split_person_name(explicit_recipient_name)
    elif source_full_name and (not first_name or not last_name):
...
def _apply_facility_pickup_recipient_name(address: object, recipient_name: str) -> object:
    ...
    return {
        **address,
        ...
        "orderRecipientName": recipient_name,
        "order_recipient_name": recipient_name,
...
def _max_attempts_for_endpoint(endpoint: str) -> int:
    endpoint = (endpoint or "").lstrip("/")
    if re.match(r"^products/[^/]+$", endpoint):
        return max(1, min(int(os.environ.get("WOO_PRODUCT_DETAIL_MAX_ATTEMPTS", "1").strip() or 1), 3))
    if re.match(r"^products/[^/]+/variations", endpoint):
        return max(1, min(int(os.environ.get("WOO_VARIATIONS_MAX_ATTEMPTS", "1").strip() or 1), 3))
    if endpoint.startswith("products"):
        return max(1, min(int(os.environ.get("WOO_PRODUCTS_MAX_ATTEMPTS", "2").strip() or 2), 4))
    ...
    if force_fresh:
        ...
        if event is not None and cooldown_seconds <= 0:
            ...
            with _catalog_cache_lock:
                ...
                if cached and cached.get("data") is not None:
                    return cached.get("data"), {"cache": "FORCE_INFLIGHT", "ttlSeconds": ttl_seconds, "noStore": True}
                inflight = _inflight.get(cache_key)
                if inflight and inflight.get("error") is not None:
                    raise inflight.get("error")
                if inflight and inflight.get("data") is not None:
                    return inflight.get("data"), {"cache": "FORCE_INFLIGHT", "ttlSeconds": ttl_seconds, "noStore": True}
        ...
        with _catalog_cache_lock:
            ...
            if inflight:
                ...
                _inflight[cache_key] = {"event": event, "data": None, "error": None, "leader": True}
                is_leader = True

        if is_leader:
            try:
                data = _fetch_catalog_http(endpoint, params, acquire_timeout=1)
                _clear_proxy_failure(cache_key)
                now_ms = _now_ms()
                expires_at = now_ms + ttl_seconds * 1000
                with _catalog_cache_lock:
...
def build_order_payload(order: Dict, customer: Dict) -> Dict:
    ...
    if is_facility_pickup:
        shipping_pickup_recipient_name = _first_non_empty_text(
            ...
            shipping_address.get("customerName") if isinstance(shipping_address, dict) else None,
            shipping_address.get("customer_name") if isinstance(shipping_address, dict) else None,
            shipping_address.get("doctorName") if isinstance(shipping_address, dict) else None,
            shipping_address.get("doctor_name") if isinstance(shipping_address, dict) else None,
        ...
        billing_pickup_recipient_name = _first_non_empty_text(
            ...
            billing_address.get("customerName") if isinstance(billing_address, dict) else None,
            billing_address.get("customer_name") if isinstance(billing_address, dict) else None,
            billing_address.get("doctorName") if isinstance(billing_address, dict) else None,
            billing_address.get("doctor_name") if isinstance(billing_address, dict) else None,
