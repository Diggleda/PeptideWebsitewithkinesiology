...
def _get_logo_data_url() -> Optional[str]:
    return _load_static_data_url(
        "logo",
        ["public/turfusionlabsphysiciansportal.png", "public/turfusionlabsphysiciansportal.png"],
        ["pep", "fulllogo"],
        "image/png",
    )
    ...
    <div class="page">
      <div class="hero">
        <div class="hero-brand">
          {f'<img class="brand-logo" src="{_escape_html(logo_data_url)}" alt="TrufusionLabs" />' if logo_data_url else '<div class="brand">TrufusionLabs</div>'}
      ...
      <div class="meta-grid">
        ...
        <div class="meta-card">
          ...
          <div class="meta-value">
            <div>{_escape_html(sales_rep.get("name") or "TrufusionLabs")}</div>
...
def _run_system_browser_renderer(quote: Dict) -> Optional[Dict]:
    ...
    with tempfile.TemporaryDirectory(prefix="trufusion-quote-pdf-") as temp_dir:
...
def _build_quote_filename(quote: Dict) -> str:
    ...
    try:
        ...
        revision = 1
    return f"TruFusion_Labs_Quote_{_sanitize_filename(prospect_name)}_{revision}.pdf"
