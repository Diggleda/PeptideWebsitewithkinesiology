...
try:
    ...
    HAS_FLASK = False
    FLASK_SKIP_REASON = f"python deps not installed: {exc}"
else:
    HAS_FLASK = True
    FLASK_SKIP_REASON = ""
...
class WebBackgroundJobsModeTests(unittest.TestCase):
    ...
    def test_web_background_jobs_mode_can_default_to_external(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            self.assertEqual(
                python_backend._resolve_web_background_jobs_mode(default_mode="external"),
                "external",
            )
    ...
    def test_web_background_jobs_mode_explicit_thread_overrides_external_default(self) -> None:
        with patch.dict(os.environ, {"TRUFUSION_WEB_BACKGROUND_JOBS_MODE": "thread"}, clear=True):
            self.assertEqual(
                python_backend._resolve_web_background_jobs_mode(default_mode="external"),
                "thread",
            )
...
@unittest.skipUnless(HAS_FLASK, FLASK_SKIP_REASON)
class AppBootstrapModeTests(unittest.TestCase):
    ...
    def test_production_create_app_defaults_to_external_background_jobs(self) -> None:
        order: list[str] = []
        config = SimpleNamespace(
            flask_settings={},
            port=3001,
        ...
        with patch.dict(os.environ, {}, clear=True), \
            ...
            patch.object(logging_config_module, "configure_logging"), \
            patch.object(services_module, "configure_services"), \
            patch.object(database_module, "init_database"), \
            patch.object(storage_module, "init_storage"), \
            patch.object(sales_prospect_repository, "ensure_house_sales_rep_for_contact_forms"), \
            patch.object(user_repository, "backfill_contact_form_lead_types"), \
            patch.object(product_document_sync_service, "start_product_document_sync", side_effect=lambda: order.append("product")), \
            patch.object(shipstation_status_sync_service, "start_shipstation_status_sync", side_effect=lambda: order.append("shipstation")), \
            ...
            patch.object(shadow_mode_module, "init_shadow_mode"), \
            patch.object(routes_module, "register_blueprints"):
            app = python_backend.create_app()

        self.assertEqual(app.config["WEB_BACKGROUND_JOBS_MODE"], "external")
