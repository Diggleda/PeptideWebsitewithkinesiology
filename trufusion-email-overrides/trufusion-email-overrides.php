...
function trufusion_email_overrides_get_constant($primary, $legacy = '', $fallback = '') {
  if (defined($primary)) {
...
add_action('plugins_loaded', function () {
  ...
  add_action('init', function () {
    ...
    add_action('woocommerce_email_before_order_table', 'trufusion_bacs_email_instructions', 10, 4);
    add_action('woocommerce_thankyou_bacs', 'trufusion_bacs_thankyou_instructions', 10);
  });
});
...
function trufusion_email_overrides_shared_users_table() {
  ...
  // define('TRUFUSION_SHARED_USERS_TABLE', 'users');
  $value = trufusion_email_overrides_get_constant('TRUFUSION_SHARED_USERS_TABLE', 'PEPPR_SHARED_USERS_TABLE', 'users');
  $value = trim($value);
  return $value !== '' ? $value : 'users';
}

function trufusion_email_overrides_sql_ident($value) {
...
function trufusion_email_overrides_table_sql($table) {
  ...
  foreach ($parts as $part) {
    $safe = trufusion_email_overrides_sql_ident($part);
    if ($safe === '') return '';
    $quoted[] = "`{$safe}`";
  }
...
function trufusion_email_overrides_shared_users_table_sql() {
  return trufusion_email_overrides_table_sql(trufusion_email_overrides_shared_users_table());
}

function trufusion_email_overrides_db_reset_last_error($db) {
  if (is_object($db) && property_exists($db, 'last_error')) {
    $db->last_error = '';
  }
}

function trufusion_email_overrides_debug_enabled() {
  return (defined('TRUFUSION_EMAIL_OVERRIDES_DEBUG') && TRUFUSION_EMAIL_OVERRIDES_DEBUG)
    || (defined('PEPPR_EMAIL_OVERRIDES_DEBUG') && PEPPR_EMAIL_OVERRIDES_DEBUG);
}

function trufusion_email_overrides_log($event, $context = array()) {
  if (!trufusion_email_overrides_debug_enabled()) return;
...
function trufusion_email_overrides_cc_fail_open() {
  return (defined('TRUFUSION_EMAIL_OVERRIDES_CC_FAIL_OPEN') && TRUFUSION_EMAIL_OVERRIDES_CC_FAIL_OPEN)
    || (defined('PEPPR_EMAIL_OVERRIDES_CC_FAIL_OPEN') && PEPPR_EMAIL_OVERRIDES_CC_FAIL_OPEN);
}

function trufusion_email_overrides_get_query_db() {
  static $cached = false;
  static $db = null;

  if ($cached) {
    trufusion_email_overrides_log('get_query_db.cached', array('has_db' => is_object($db)));
    return $db;
  }
  $cached = true;

  $host = trufusion_email_overrides_get_constant('TRUFUSION_SHARED_DB_HOST', 'PEPPR_SHARED_DB_HOST', '');
  $port = (int) trufusion_email_overrides_get_constant('TRUFUSION_SHARED_DB_PORT', 'PEPPR_SHARED_DB_PORT', '0');
  $user = trufusion_email_overrides_get_constant('TRUFUSION_SHARED_DB_USER', 'PEPPR_SHARED_DB_USER', '');
  $pass = trufusion_email_overrides_get_constant('TRUFUSION_SHARED_DB_PASS', 'PEPPR_SHARED_DB_PASS', '');
  $name = trufusion_email_overrides_get_constant('TRUFUSION_SHARED_DB_NAME', 'PEPPR_SHARED_DB_NAME', '');
  ...
  if ($host !== '' && $user !== '' && $name !== '' && class_exists('wpdb')) {
    ...
    if (is_object($candidate)) {
      ...
      $charset = trufusion_email_overrides_get_constant('TRUFUSION_SHARED_DB_CHARSET', 'PEPPR_SHARED_DB_CHARSET', defined('DB_CHARSET') ? (string) DB_CHARSET : '');
      $collate = trufusion_email_overrides_get_constant('TRUFUSION_SHARED_DB_COLLATE', 'PEPPR_SHARED_DB_COLLATE', '');
      ...
      trufusion_email_overrides_db_reset_last_error($candidate);
      $probe = method_exists($candidate, 'get_var') ? $candidate->get_var('SELECT 1') : null;
      if ((string) $probe === '1' && empty($candidate->last_error)) {
        $db = $candidate;
        trufusion_email_overrides_log('get_query_db.shared_connected', array(
          'host' => $host_with_port,
          'database' => $name,
        ));
        return $db;
      }
      trufusion_email_overrides_log('get_query_db.shared_failed', array(
        'host' => $host_with_port,
        'database' => $name,
        'probe' => $probe,
  ...
  if (isset($wpdb) && is_object($wpdb)) {
    $db = $wpdb;
    trufusion_email_overrides_log('get_query_db.fallback_wpdb');
  } else {
    trufusion_email_overrides_log('get_query_db.no_db');
  }

  return $db;
}

function trufusion_email_overrides_boolish($value) {
...
function trufusion_email_overrides_rep_cc_enabled($rep_email) {
  $rep_email = strtolower(trim((string) $rep_email));
  if ($rep_email === '' || strpos($rep_email, '@') === false) return false;

  $fail_open = trufusion_email_overrides_cc_fail_open();

  $db = trufusion_email_overrides_get_query_db();
  if (!isset($db) || !is_object($db) || !method_exists($db, 'get_var')) {
    trufusion_email_overrides_log('rep_cc_enabled.no_db', array(
      'rep_email' => $rep_email,
      'decision' => $fail_open,
      'fail_open' => $fail_open,
    ));
  ...
  $table_sql = trufusion_email_overrides_shared_users_table_sql();
  if ($table_sql === '') {
    trufusion_email_overrides_log('rep_cc_enabled.bad_table', array('table' => trufusion_email_overrides_shared_users_table()));
    return $fail_open;
  }

  trufusion_email_overrides_db_reset_last_error($db);
  $query = $db->prepare(
  ...
  if (!empty($db->last_error)) {
    trufusion_email_overrides_log('rep_cc_enabled.query_error', array(
      'table' => $table_sql,
      'rep_email' => $rep_email,
  ...
  if ($raw === null) {
    ...
    trufusion_email_overrides_log('rep_cc_enabled.row_missing', array(
      'table' => $table_sql,
      'rep_email' => $rep_email,
      'decision' => $fail_open,
      'fail_open' => $fail_open,
  ...
  $enabled = trufusion_email_overrides_boolish($raw);
  trufusion_email_overrides_log('rep_cc_enabled.result', array(
    'table' => $table_sql,
    'rep_email' => $rep_email,
    'raw' => $raw,
    'enabled' => $enabled,
...
function trufusion_email_overrides_lookup_rep_email_by_id($rep_id) {
  ...
  $table_sql = trufusion_email_overrides_shared_users_table_sql();
  if ($table_sql === '') {
    trufusion_email_overrides_log('lookup_by_id.bad_table', array('rep_id' => $rep_id, 'table' => trufusion_email_overrides_shared_users_table()));
    return '';
  }

  foreach (array('sales_rep_id', 'salesRepId', 'id', 'legacy_user_id', 'legacyUserId') as $column) {
    $safe_column = trufusion_email_overrides_sql_ident($column);
    ...
    if (!empty($db->last_error)) {
      trufusion_email_overrides_log('lookup_by_id.column_error', array(
        'table' => $table_sql,
        'column' => $safe_column,
        'rep_id' => $rep_id,
        'last_error' => (string) $db->last_error,
    ...
    if ($email !== '') {
      trufusion_email_overrides_log('lookup_by_id.found', array(
        'table' => $table_sql,
        'column' => $safe_column,
        'rep_id' => $rep_id,
        'rep_email' => $email,
  ...
  trufusion_email_overrides_log('lookup_by_id.not_found', array('table' => $table_sql, 'rep_id' => $rep_id));
  return '';
}

function trufusion_email_overrides_lookup_rep_email_by_customer_email($customer_email) {
  ...
  $table_sql = trufusion_email_overrides_shared_users_table_sql();
  if ($table_sql === '') {
    trufusion_email_overrides_log('lookup_by_customer.bad_table', array(
      'customer_email' => $customer_email,
      'table' => trufusion_email_overrides_shared_users_table(),
    ));
    return '';
  }
  ...
  foreach (array('sales_rep_email', 'salesRepEmail', 'doctor_sales_rep_email', 'doctorSalesRepEmail') as $column) {
    ...
    $email = sanitize_email((string) $db->get_var($query));
    if (!empty($db->last_error)) {
      trufusion_email_overrides_log('lookup_by_customer.email_column_error', array(
        'table' => $table_sql,
        'column' => $safe_column,
        'customer_email' => $customer_email,
        'last_error' => (string) $db->last_error,
    ...
    if ($email !== '') {
      trufusion_email_overrides_log('lookup_by_customer.email_found', array(
        'table' => $table_sql,
        'column' => $safe_column,
        'customer_email' => $customer_email,
        'rep_email' => $email,
  ...
  foreach (array('sales_rep_id', 'salesRepId', 'doctor_sales_rep_id', 'doctorSalesRepId') as $column) {
    ...
    if (!empty($db->last_error)) {
      trufusion_email_overrides_log('lookup_by_customer.id_column_error', array(
        'table' => $table_sql,
        'column' => $safe_column,
        'customer_email' => $customer_email,
        'last_error' => (string) $db->last_error,
    ...
    if ($rep_id !== '') {
      trufusion_email_overrides_log('lookup_by_customer.id_found', array(
        'table' => $table_sql,
        'column' => $safe_column,
        'customer_email' => $customer_email,
        'rep_id' => $rep_id,
      ));
      return trufusion_email_overrides_lookup_rep_email_by_id($rep_id);
    }
  }

  trufusion_email_overrides_log('lookup_by_customer.not_found', array(
    'table' => $table_sql,
    'customer_email' => $customer_email,
  ));
  return '';
...
function trufusion_email_overrides_resolve_rep_email($order) {
  ...
  $rep_id = trim((string) $order->get_meta('trufusion_sales_rep_id'));
  if ($rep_id !== '') {
  ...
  trufusion_email_overrides_log('resolve_rep_email.not_found', array(
    'order_id' => (int) $order->get_id(),
    'rep_id' => $rep_id,
    'customer_email' => $customer_email,
  ));
...
function trufusion_email_overrides_add_sales_rep_cc($headers, $email_id, $order, $email) {
  ...
  if ($customer_email !== '' && strtolower($rep_email) === $customer_email) {
    trufusion_email_overrides_log('add_cc.skip.same_as_customer', array(
      'order_id' => (int) $order->get_id(),
      'customer_email' => $customer_email,
      'rep_email' => $rep_email,
    ));
  ...
  $opted_in = trufusion_email_overrides_rep_cc_enabled($rep_email);
  if (!$opted_in) {
    trufusion_email_overrides_log('add_cc.skip.opted_out', array(
      'order_id' => (int) $order->get_id(),
      'rep_email' => $rep_email,
    ));
    return $headers;
...
function trufusion_email_overrides_render_discount_summary($order, $sent_to_admin, $plain_text, $email) {
  ...
  $discount_code = strtoupper(trim((string) $order->get_meta('trufusion_discount_code')));
...
add_action('woocommerce_email_after_order_table', 'trufusion_email_overrides_render_discount_summary', 20, 4);

function trufusion_bacs_email_instructions($order, $sent_to_admin, $plain_text, $email) {
...
function trufusion_bacs_thankyou_instructions($order_id) {
